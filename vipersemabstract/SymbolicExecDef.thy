...
"rtc_and rt1 rt2 = (fst rt1 \<and> fst rt2, 
  ...
  (Stmt rtcs1, Stmt rtcs2) \<Rightarrow> Stmt (rtcs1 @ rtcs2)
  | (_, _) \<Rightarrow> Stmt []))"
...
"rtc_add rtc st =
  (case st of
  Stmt rtcl \<Rightarrow> Stmt (rtc # rtcl)
  | _ \<Rightarrow> Stmt [])"
...
"proto_heap_rem_acc h pc te f p =
  (let (h, v, q) = 
    foldl (\<lambda> (h, res, d) c. 
      if \<not> (alias_chunk1 c pc te f) then (c # h, res, d)
      else (if (pc \<turnstile>\<^sub>s d <\<^sub>s chunk_perm c)
        then (c\<lparr> chunk_perm := chunk_perm c -\<^sub>s d\<rparr> # h, chunk_val c, SPerm 0)
        else (h, res, d -\<^sub>s chunk_perm c)))
    ([], SNull, p)
    h
    in (h, v, pc \<turnstile>\<^sub>s SPerm 0 <\<^sub>s q))"
...
"sym_exp_acc_helper \<sigma> te f Q =
  rtc_or (
      rtc_or (
      (sym_heap_extract \<sigma> te f (Some (SPerm 0))
        (\<lambda> \<sigma> c. sym_heap_do_add \<sigma> c (\<lambda> \<sigma>. Q \<sigma> (chunk_val c)))))
      (sym_opheap_extract \<sigma> te f (Some (SPerm 0))
        (\<lambda> \<sigma> c. sym_opheap_do_add \<sigma> c (\<lambda> \<sigma>. Q \<sigma> (chunk_val c)))))
      (sym_imprecise_rtc \<sigma> te f Q)"
...
(*
consume(sigma, f?, h?, h, acc(e.f, p), Q) =
 eval-c(sigma, p::e, (lambda sigma, p'::t.
  if(not imprecise)
    do regular sconsume code
  else
    (h1, delta1, b1) = heap-rem-acc(h, pi, f(t), p')
    (h?', d2, b2) = heap-rem-acc(h?, pi, f(t), p')
    if(b1)
      runtime check or no?
*)

definition sym_consume_helper :: "'a sym_state \<Rightarrow> 'a sym_exp \<Rightarrow> field_name \<Rightarrow> 'a sym_exp \<Rightarrow> ('a sym_state \<Rightarrow> 'a ret_typ) \<Rightarrow> 'a ret_typ" where
"sym_consume_helper \<sigma> te f tep Q = 
...
| "sexec_exp_p \<sigma> (Unop op e) Q = sexec_exp_p \<sigma> e (\<lambda> \<sigma> t. Q \<sigma> (SUnop op t))"
...
| "sexec_exp_c \<sigma> (Unop op e) Q = sexec_exp_c \<sigma> e (\<lambda> \<sigma> t. Q \<sigma> (SUnop op t))"
...
| "sproduce \<sigma> (Atomic (Acc e f (PureExp ep))) Q =
    ...
    let res = (let ty = SOME ty. sym_fields \<sigma> f = Some ty in
       sym_gen_fresh \<sigma> ty (\<lambda> \<sigma> tv.
       sym_heap_do_add \<sigma> 
        \<lparr> chunk_field = f, chunk_recv = te, chunk_perm = tep, chunk_val = (SVar tv) \<rparr> Q))
    in
    ((sym_cond \<sigma> \<turnstile>\<^sub>s \<not>\<^sub>s (tep =\<^sub>s SPerm 0)) \<and> fst res, snd res)))"
...
fun sconsume :: "'a sym_state \<Rightarrow> (pure_exp, pure_exp atomic_assert) assert \<Rightarrow> ('a sym_state \<Rightarrow> 'a ret_typ) \<Rightarrow> 'a ret_typ" where
  "sconsume \<sigma> (Atomic (Pure e)) Q = 
    sexec_exp_c \<sigma> e (\<lambda> \<sigma> t. ((sym_cond \<sigma> \<turnstile>\<^sub>s t) \<and> fst (Q \<sigma>), snd (Q \<sigma>)))"
